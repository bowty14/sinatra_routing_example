...
describe "#Song" do
  ...
  describe('.all') do
    it("returns and empty array if there are no songs") do
  ...
  describe('.find_by_album') do
    it("finds songs for an album") do
      ...
      album2.save()
      song = Song.new({:name => "Naima", :album_id => album2.id, :id => nil})
      song.save()
      song2 = Song.new({:name => "California", :album_id => album2.id, :id => nil})
      song2.save()
      expect(Song.find_by_album(album2.id)).to(eq([song, song2]))
    end
  end
end
